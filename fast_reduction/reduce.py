"""
Reduction primitives following the H100 memory hierarchy:

  thread reduction  (registers)
  warp reduction    (registers, shuffle)
  block reduction   (shared memory)
  cluster reduction (distributed shared memory)

Adapted from quack/reduce.py (Dao 2025).
"""
...
def block_reduce(
    ...
    reduction_buffer shape: (num_warps / warps_per_row, warps_per_row)
    """
...
def cluster_reduce(
    ...
    """Cluster-level reduction via distributed shared memory (Hopper+).

    Each warp sends its reduced value to all peer CTAs via store_shared_remote,
    then all lanes reduce across the full buffer.
...
def block_or_cluster_reduce(
    ...
    """Dispatch to block or cluster reduction based on whether mbar_ptr is provided."""
...
# ---- row_reduce: the main entry point ----
...
def row_reduce(
    ...
    """Full row reduction: thread -> warp -> block/cluster.
...
# ---- online softmax: fused max + sum_exp in one pass ----
...
def online_softmax_reduce(
    ...
    """Online softmax: compute (max_x, sum_exp) in one pass.

    When multiple warps or cluster CTAs participate, packs (max, sum_exp) into
    Int64 for a single reduction, then unpacks and adjusts.

    reduction_buffer shape: (rows, (warps_per_row, cluster_n))
    reduction_buffer element_type must be Int64.
    """
