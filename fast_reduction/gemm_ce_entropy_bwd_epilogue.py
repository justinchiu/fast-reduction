"""
GEMM epilogue mixin for fused CE+entropy with logits output (for backward).

Same reduction logic as GemmCEEntropyEpiMixin (v5.1), but also writes fp32
logits to a GMEM buffer during Pass 1.  This lets the backward skip cuBLAS
logits recomputation and logsumexp recomputation — both come from WGMMA.

Extra epilogue argument: mLogits (M, V) fp32.
Extra cost: ~2.1 GB of uncoalesced global stores per chunk, overlapped with
WGMMA matmul of subsequent tiles.
"""
...
class GemmCEEntropyBwdEpiMixin(GemmDefaultEpiMixin):
    """Custom epilogue: reduces acc to partial CE+entropy stats + writes logits."""
    ...
    class EpilogueArguments(ArgumentsBase):
        ...
        mLogits: cute.Tensor      # (M, V) fp32 logits output
    ...
    def epi_smem_bytes_per_stage(
        args: "GemmCEEntropyBwdEpiMixin.EpilogueArguments",
            ...
            for epi_n in cutlass.range_constexpr(n_epi_n):
                ...
                if tidx < epi_M:
                    ...
                    if global_m < total_M:
                        ...
                        # Pass 1: row max + target logit + write logits to GMEM
                        ...
                        for j in cutlass.range(epi_N, unroll=8):
                            ...
                            if global_n < V:
                                ...
                                if global_n == target_idx:
                                    partial_target = val
                                # Write logit to GMEM
                                params.mLogits[global_m, global_n] = val
...
class GemmCEEntropyBwdSm90(GemmCEEntropyBwdEpiMixin, GemmSm90):
    pass
