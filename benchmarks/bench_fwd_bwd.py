...
def main():
    ...
    # ---- 4F. CuTe joint fast (interleaved fwd+bwd) ----
    def run_fused_fast_fwd_bwd():
        h = hidden.detach().requires_grad_(True)
        w = weight.detach().requires_grad_(True)
    ...
    fwd_bwd_ms, peak = benchmark_fn(run_fused_fast_fwd_bwd, args.warmup, args.iters)
    ce_mae = ent_mae = dh_mae = None
    if ce_ref is not None:
        ce, ent, h, w = run_fused_fast_fwd_bwd()
        ce_mae = (ce.float().cpu() - ce_ref).abs().mean().item()
        ent_mae = (ent.float().cpu() - ent_ref).abs().mean().item()
        dh_mae = (h.grad.float().cpu() - d_hidden_ref.float()).abs().mean().item()
    report("4F. CuTe joint fast", 0, fwd_bwd_ms, peak, ce_mae, ent_mae, dh_mae)

    # ---- 5F. GEMM epilogue fast (interleaved fwd+bwd) ----
    def run_gemm_fast_fwd_bwd():
        h = hidden.detach().requires_grad_(True)
        w = weight.detach().requires_grad_(True)
    ...
    fwd_bwd_ms, peak = benchmark_fn(run_gemm_fast_fwd_bwd, args.warmup, args.iters)
    ce_mae = ent_mae = dh_mae = None
    if ce_ref is not None:
        ce, ent, h, w = run_gemm_fast_fwd_bwd()
        ce_mae = (ce.float().cpu() - ce_ref).abs().mean().item()
        ent_mae = (ent.float().cpu() - ent_ref).abs().mean().item()
        dh_mae = (h.grad.float().cpu() - d_hidden_ref.float()).abs().mean().item()
